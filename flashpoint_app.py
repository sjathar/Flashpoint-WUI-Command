"""
Flashpoint: WUI Command (preserved prototype)

Run locally with:  streamlit run flashpoint_app.py
...
from local_store import LocalClient, get_local_client
...
def _mark_local_mode(reason: str) -> LocalClient:
    st.session_state["local_mode"] = True
    st.session_state["local_mode_reason"] = reason
    return get_local_client()


def get_supabase() -> Client | LocalClient:
    """Return a live backend: local SQLite, or Supabase when it is reachable."""
    if _flag("USE_LOCAL_DB", default=True):
        return _mark_local_mode(
            "Local play mode — no internet required. Game data is stored on this computer."
        )
    ...
    try:
        ...
        client.table("global_state").select("id").eq("id", 1).limit(1).execute()
...
@st.cache_data(ttl=2)
def fetch_global_state(_sb: Client | LocalClient) -> dict[str, Any]:
    def _run():
...
@st.cache_data(ttl=5)
def fetch_grid(_sb: Client | LocalClient) -> list[dict[str, Any]]:
    def _run():
...
def fetch_player_count(sb: Client | LocalClient) -> int:
    def _run():
...
def fetch_player(sb: Client | LocalClient, player_id: str) -> dict[str, Any] | None:
    def _run():
        res = (
            sb.table("players")
    ...
    return _supabase_call("loading your tile", _run)
...
def main() -> None:
    ...
    sb = get_supabase()
    if st.session_state.get("local_mode"):
        st.info(
